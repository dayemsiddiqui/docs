...
### Type-Safe Forms with useForm

Use the generated type with Inertia's `useForm` hook for full type safety:
...
export default function CreateTodo() {
  // Full type safety - TypeScript knows the shape of your form data
  const { data, setData, post, processing, errors } = useForm<CreateTodoRequest>({
    title: '',
    description: null,
  })
  ...
  return (
    <form onSubmit={handleSubmit}>
      <input
        type="text"
        value={data.title}
        onChange={(e) => setData('title', e.target.value)} // Type-safe!
