...
description: 'Schedule recurring tasks with Kit Laravel-like scheduler'
icon: 'clock'
---
...
Kit supports two approaches for defining scheduled tasks:

### 1. Trait-Based Tasks (Recommended)

For complex tasks that need dependencies or reusable logic, implement the `ScheduledTask` trait:

```rust
// src/tasks/cleanup_logs_task.rs
use async_trait::async_trait;
use kit::{CronExpression, FrameworkError, ScheduledTask, App, DB};
...
impl ScheduledTask for CleanupLogsTask {
    ...
    fn description(&self) -> Option<&str> {
        Some("Removes logs older than 30 days")
    }
}
```
...
```rust
#[async_trait]
impl ScheduledTask for LongRunningTask {
    // ... other methods
...
```rust
#[async_trait]
impl ScheduledTask for BackgroundTask {
    // ... other methods
...
#[async_trait]
impl ScheduledTask for SendRemindersTask {
    async fn handle(&self) -> Result<(), FrameworkError> {
        // Access database
...
| Trait-based | Implement `ScheduledTask` trait |
| Closure-based | `schedule.call(\|\| async { ... })` |
| Register tasks | Add to `src/schedule.rs` |
