...
import type { TodoRequest } from '../../types/inertia-props'

export default function TodoCreate() {
  // Type-safe form using the generated TodoRequest type
  const { data, setData, post, processing, errors } = useForm<TodoRequest>({
    title: '',
  })
...
```tsx
import { Link, useForm } from '@inertiajs/react'
import type { TodoFormProps, TodoRequest } from '../../types/inertia-props'

export default function TodoEdit({ todo }: TodoFormProps) {
  // Type-safe form using the generated TodoRequest type
  const { data, setData, put, processing, errors } = useForm<TodoRequest>({
    title: todo?.title || '',
  })
