...
Kit integrates [Inertia.js](https://inertiajs.com/) to provide a seamless way to build modern single-page applications using React, without the complexity of building an API.
...
```rust
use kit::{Request, Response, inertia_response};
...
INERTIA_DEVELOPMENT=false kit serve --backend-only
```
